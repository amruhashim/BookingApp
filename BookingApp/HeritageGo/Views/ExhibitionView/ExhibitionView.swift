...
struct ExhibitionsView: View {
    var cardHeight: CGFloat // Height of each exhibition card
    var exhibition = ["Art Gallery", "WWI Exhibition", "Exploring the Space", "Visual Show"] // List of exhibitions
    
    @State private var showModal = false // State to control modal presentation
    @State private var selected = "" // Selected exhibition
    ...
    var body: some View {
        ScrollView(.vertical, showsIndicators: false) {
            LazyVGrid(columns: columns, spacing: 10) {
                ForEach(exhibition, id: \.self) { loc in
                    Button(action: {
                        self.showModal = true
                        self.selected = loc
                    }) {
                    ...
                    .sheet(isPresented: self.$showModal) {
                        ExhibitionDetailView(event: $selected, show: $showModal)
                    }
                }
            }
